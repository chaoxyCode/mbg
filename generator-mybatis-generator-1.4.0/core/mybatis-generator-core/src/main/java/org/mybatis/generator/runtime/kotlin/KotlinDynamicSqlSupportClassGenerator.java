...
public class KotlinDynamicSqlSupportClassGenerator {
    ...
    public KotlinDynamicSqlSupportClassGenerator(
    ...
    private KotlinType buildOuterObject(KotlinFile kotlinFile, FullyQualifiedJavaType type) {
        KotlinType outerObject =
                KotlinType.newObject(type.getShortNameWithoutTypeArguments()).build();
    ...
    private void handleColumn(
            KotlinFile kotlinFile, KotlinType kotlinType, IntrospectedColumn column) {
        ...
        KotlinProperty property =
                KotlinProperty.newVal(fieldName)
    ...
    private String calculateInnerInitializationString(
            IntrospectedColumn column, FullyQualifiedKotlinType kt) {
        StringBuilder initializationString = new StringBuilder();

        initializationString.append(
                String.format(
                        "column<%s>(\"%s\", JDBCType.%s", //$NON-NLS-1$
