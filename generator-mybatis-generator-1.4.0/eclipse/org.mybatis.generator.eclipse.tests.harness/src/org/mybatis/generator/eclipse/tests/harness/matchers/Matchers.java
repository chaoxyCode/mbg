...
/**
 * This class holds static methods that create matchers. This is in keeping with the normal hamcrest
 * pattern.
 *
 * <p>Many of the methods are expressed in two ways: hasXXX and withXXX. These methods will return
 * equivalent matchers. They exist to allow for more expressive unit tests when chaining matchers.
 * For example you can write this: <code>
 * assertThat(summary, hasClass("OuterClass", withClass("InnerClass")))</code> This test checks that
 * the summary object contains a class named OuterClass and that the class OuterClass contains an
 * inner class named InnerClass. The <code>hasClass</code> and <code>withClass</code> methods return
 * equivalent matchers, but the test is more expressive.
 *
 * @author Jeff Butler
 */
