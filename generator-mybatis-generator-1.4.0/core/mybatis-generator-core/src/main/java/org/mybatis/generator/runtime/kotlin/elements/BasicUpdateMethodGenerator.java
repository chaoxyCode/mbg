...
public class BasicUpdateMethodGenerator extends AbstractKotlinFunctionGenerator {
        ...
        KotlinFunctionAndImports functionAndImports =
                KotlinFunctionAndImports.withFunction(
                                KotlinFunction.newOneLineFunction("update") // $NON-NLS-1$
                                        ...
                                        .withArgument(
                                                KotlinArg.newArg("updateStatement") // $NON-NLS-1$
                                                        .withDataType(
                                                                "UpdateStatementProvider") //$NON-NLS-1$
                                                        .build())
                                        .withAnnotation(
                                                "@UpdateProvider(type=SqlProviderAdapter::class, method=\"update\")") //$NON-NLS-1$
                                        .build())
                        .withImport(
    ...
    public boolean callPlugins(KotlinFunction kotlinFunction, KotlinFile kotlinFile) {
        return context.getPlugins()
                .clientBasicUpdateMethodGenerated(kotlinFunction, kotlinFile, introspectedTable);
    }
