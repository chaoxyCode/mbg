...
public class GeneralSelectMethodGenerator extends AbstractKotlinFunctionGenerator {
        ...
        KotlinFunctionAndImports functionAndImports =
                KotlinFunctionAndImports.withFunction(
                                KotlinFunction.newOneLineFunction(
                                                mapperName + ".select") // $NON-NLS-1$
                                        .withArgument(
                                                KotlinArg.newArg("completer") // $NON-NLS-1$
                                                        .withDataType(
                                                                "SelectCompleter") //$NON-NLS-1$
                                                        .build())
                                        .withCodeLine(
                                                "selectList(this::selectMany, columnList, "
    ...
    public boolean callPlugins(KotlinFunction kotlinFunction, KotlinFile kotlinFile) {
        return context.getPlugins()
                .clientGeneralSelectMethodGenerated(kotlinFunction, kotlinFile, introspectedTable);
    }
