...
public class ColumnListGenerator {
    ...
    public KotlinPropertyAndImports generatePropertyAndImports() {
        KotlinPropertyAndImports propertyAndImports =
                KotlinPropertyAndImports.withProperty(
        ...
        context.getCommentGenerator()
                .addGeneralPropertyComment(
                        propertyAndImports.getProperty(),
                        introspectedTable,
    ...
    private String getInitializationString() {
        return introspectedTable.getAllColumns().stream()
                ...
                .collect(
                        Collectors.joining(
                                ", ", "listOf(", ")")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    public boolean callPlugins(KotlinProperty kotlinProperty, KotlinFile kotlinFile) {
        return context.getPlugins()
                .clientColumnListPropertyGenerated(kotlinProperty, kotlinFile, introspectedTable);
    }
