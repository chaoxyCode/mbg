...
/**
 * This class generates an Ant file that can be used to run the generator from the launcher.
 *
 * @author Jeff Butler
 */
public class AntFileGenerator implements GeneratorLaunchConstants {
    ...
    private XmlElement getProjectElement() {
        ...
        projectElement.addAttribute(
                new Attribute("default", "generate")); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    private void addSqlTaskIfNecessary(XmlElement parent) {
        ...
        if (sqlFileFullPath != null) {
            ...
            sqlTask.addAttribute(
                    new Attribute(
                            "driver",
                            getTextOrBlank(
                                    configuration, ATTR_SQL_SCRIPT_DRIVER_CLASS))); // $NON-NLS-1$
            sqlTask.addAttribute(
                    new Attribute(
                            "url",
                            getTextOrBlank(
                                    configuration, ATTR_SQL_SCRIPT_CONNECTION_URL))); // $NON-NLS-1$
            sqlTask.addAttribute(
                    new Attribute("userid", LauncherUtils.getUserId(configuration))); // $NON-NLS-1$
            sqlTask.addAttribute(
                    new Attribute(
                            "password", LauncherUtils.getPassword(configuration))); // $NON-NLS-1$
