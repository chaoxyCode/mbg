...
public class KotlinDataClassGenerator extends AbstractKotlinGenerator {
    ...
    public List<KotlinFile> getKotlinFiles() {
        ...
        FullyQualifiedKotlinType type =
                new FullyQualifiedKotlinType(introspectedTable.getKotlinRecordType());

        KotlinFile kf = new KotlinFile(type.getShortNameWithoutTypeArguments());
        kf.setPackage(type.getPackageName());

        KotlinType dataClass =
                KotlinType.newClass(type.getShortNameWithoutTypeArguments())
        ...
        for (IntrospectedColumn introspectedColumn : introspectedColumns) {
            ...
            KotlinProperty kp =
                    KotlinProperty.newVar(introspectedColumn.getJavaProperty())
                            .withDataType(
                                    kotlinType.getShortNameWithTypeArguments() + "?") // $NON-NLS-1$
