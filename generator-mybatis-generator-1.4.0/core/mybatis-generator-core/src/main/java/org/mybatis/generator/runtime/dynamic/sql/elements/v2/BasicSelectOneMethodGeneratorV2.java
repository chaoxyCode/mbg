...
public class BasicSelectOneMethodGeneratorV2 extends AbstractMethodGenerator {
    ...
    public MethodAndImports generateMethodAndImports() {
        ...
        FullyQualifiedJavaType annotation =
                new FullyQualifiedJavaType(
                        "org.apache.ibatis.annotations.SelectProvider"); //$NON-NLS-1$
        imports.add(annotation);

        FullyQualifiedJavaType returnType =
                new FullyQualifiedJavaType("java.util.Optional"); // $NON-NLS-1$
        ...
        method.addAnnotation(
                "@SelectProvider(type=SqlProviderAdapter.class, method=\"select\")"); //$NON-NLS-1$
        ...
        if (introspectedTable.isConstructorBased()) {
            ...
            if (reuseResultMap) {
                ...
                method.addAnnotation(
                        "@ResultMap(\"" + resultMapId + "\")"); // $NON-NLS-1$ //$NON-NLS-2$
            } else {
    ...
    public boolean callPlugins(Method method, Interface interfaze) {
        return context.getPlugins()
                .clientBasicSelectOneMethodGenerated(method, interfaze, introspectedTable);
    }
