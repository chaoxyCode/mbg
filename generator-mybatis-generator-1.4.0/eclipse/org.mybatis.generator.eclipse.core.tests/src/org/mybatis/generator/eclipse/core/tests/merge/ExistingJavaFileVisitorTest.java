...
public class ExistingJavaFileVisitorTest {
    ...
    public void testRegularClass() throws Exception {
        InputStream resource =
                getClass()
                        .getResourceAsStream(
                                "/org/mybatis/generator/eclipse/core/tests/merge/resources/AwfulTable.java.src");
        String source = getResourceAsString(resource);
        IDocument document = new Document(source);
        CompilationUnit cu = getCompilationUnitFromSource(source);

        cu.recordModifications();
        ExistingJavaFileVisitor visitor =
                new ExistingJavaFileVisitor(MergeConstants.getOldElementTags());
        ...
        assertThat(
                visitor.getTypeDeclaration().getName().getFullyQualifiedName(), is("AwfulTable"));
    ...
    public void testComplexClass() throws Exception {
        ...
        assertThat(visitor.getTypeDeclaration().isInterface(), is(false));
        assertThat(
                visitor.getTypeDeclaration().getName().getFullyQualifiedName(),
