...
/**
 * This launcher works by invoking the AntRunner on a generated ant script for the generator. It
 * turns out that calculating the classpath for running a plain Java launch configuration is an
 * incredible pain when we need to include Eclipse dependencies, and then we still have the problem
 * that the Java launch would need to be able run in the same JRE as the workspace so that it could
 * resolve Eclipse projects and directories. In the end, it is far simpler to reuse the built in Ant
 * support which handles most of those issues automatically.
 *
 * @author Jeff Butler
 */
public class GeneratorLaunchConfigurationDelegate extends AbstractJavaLaunchConfigurationDelegate {
    ...
    public void launch(
        ...
        try {
            ...
            Status status =
                    new Status(
                            Status.ERROR,
                            Activator.PLUGIN_ID,
                            Messages.LAUNCH_ERROR_ERROR_GENERATING_ANT_FILE,
                            e);
            throw new CoreException(status);
        }

        antRunner.setBuildFileLocation(buildFile);
        antRunner.addBuildLogger(
                "org.mybatis.generator.eclipse.ui.ant.GeneratorBuildLogger"); //$NON-NLS-1$
        ...
        if (LauncherUtils.getBooleanOrFalse(
    ...
    private Map<String, String> getUserProperties(ILaunchConfiguration configuration)
            throws CoreException {
    ...
    private void modifyAntClasspathIfNecessary(
            ILaunchConfiguration configuration, AntRunner antRunner) throws CoreException {
    ...
    private String generateAntScript(ILaunchConfiguration configuration) throws IOException {
        IPath ipath =
                Activator.getDefault()
                        .getStateLocation()
                        .append(".generatedAntScripts"); // $NON-NLS-1$
