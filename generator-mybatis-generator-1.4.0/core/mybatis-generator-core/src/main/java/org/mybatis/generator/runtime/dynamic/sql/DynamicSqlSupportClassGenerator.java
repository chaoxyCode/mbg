...
public class DynamicSqlSupportClassGenerator {
    ...
    private TopLevelClass buildBasicClass() {
        TopLevelClass topLevelClass =
                new TopLevelClass(introspectedTable.getMyBatisDynamicSqlSupportType());
        topLevelClass.setVisibility(JavaVisibility.PUBLIC);
        topLevelClass.setFinal(true);
        topLevelClass.addImportedType(
                new FullyQualifiedJavaType("org.mybatis.dynamic.sql.SqlColumn")); // $NON-NLS-1$
        topLevelClass.addImportedType(
                new FullyQualifiedJavaType("org.mybatis.dynamic.sql.SqlTable")); // $NON-NLS-1$
        topLevelClass.addImportedType(
                new FullyQualifiedJavaType("java.sql.JDBCType")); // $NON-NLS-1$
        return topLevelClass;
    }
    ...
    private InnerClass buildInnerTableClass(TopLevelClass topLevelClass) {
        ...
        commentGenerator.addClassAnnotation(
                innerClass, introspectedTable, topLevelClass.getImportedTypes());
    ...
    private Field calculateTableDefinition(TopLevelClass topLevelClass) {
        ...
        commentGenerator.addFieldAnnotation(
                field, introspectedTable, topLevelClass.getImportedTypes());
    ...
    private void handleColumn(
        ...
        if (fieldName.equals(tableFieldName)) {
            ...
            commentGenerator.addFieldAnnotation(
                    field, introspectedTable, column, topLevelClass.getImportedTypes());
    ...
    private FullyQualifiedJavaType calculateFieldType(IntrospectedColumn column) {
        ...
        return new FullyQualifiedJavaType(
                String.format("SqlColumn<%s>", typeParameter.getShortName())); // $NON-NLS-1$
    }
    ...
    public static DynamicSqlSupportClassGenerator of(
