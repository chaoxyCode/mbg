...
public class UpdateByPrimaryKeySelectiveMethodGenerator extends AbstractMethodGenerator {
    ...
    public MethodAndImports generateMethodAndImports() {
        ...
        method.addBodyLine(
                "return UpdateDSL.updateWithMapper(this::update, " //$NON-NLS-1$
                        + tableFieldName
                        + ")"); //$NON-NLS-1$

        method.addBodyLines(
                fragmentGenerator.getSetEqualWhenPresentLines(
                        introspectedTable.getNonPrimaryKeyColumns(), false));
        method.addBodyLines(
                fragmentGenerator.getPrimaryKeyWhereClauseForUpdate("        ")); // $NON-NLS-1$
    ...
    public static class Builder
            extends BaseBuilder<Builder, UpdateByPrimaryKeySelectiveMethodGenerator> {
        private FullyQualifiedJavaType recordType;
        private FragmentGenerator fragmentGenerator;
