...
public class Context extends PropertyHolder {
    ...
    /**
     * This method does a simple validate, it makes sure that all required fields have been filled
     * in. It does not do any more complex operations such as validating that database tables exist
     * or validating that named columns exist
     *
    ...
    /**
     * Introspect tables based on the configuration specified in the constructor. This method is
     * long running.
     *
    ...
    public void introspectTables(
            ProgressCallback callback, List<String> warnings, Set<String> fullyQualifiedTableNames)
        ...
        if (introspectedTables != null) {
            for (IntrospectedTable introspectedTable : introspectedTables) {
                ...
                generatedJavaFiles.addAll(
                        pluginAggregator.contextGenerateAdditionalJavaFiles(introspectedTable));
                generatedXmlFiles.addAll(
                        pluginAggregator.contextGenerateAdditionalXmlFiles(introspectedTable));
                generatedKotlinFiles.addAll(
                        pluginAggregator.contextGenerateAdditionalKotlinFiles(introspectedTable));
            }
        }
