...
public class KotlinTypeRenderer {
    ...
    public List<String> render(KotlinType kotlinType) {
        ...
        String line =
                KotlinRenderingUtilities.renderModifiers(kotlinType.getModifiers())
                        + kotlinType.getType().getValue()
                        + " " //$NON-NLS-1$
                        + kotlinType.getName();

        line +=
                kotlinType.getSuperTypes().stream()
                        .collect(
        ...
        for (KotlinNamedItem namedItem : kotlinType.getNamedItems()) {
            answer.addAll(
                    renderer.render(namedItem).stream()
                            .map(KotlinRenderingUtilities::kotlinIndent)
    ...
    private List<String> renderConstructorItems(KotlinType kotlinType) {
        ...
        while (iter.hasNext()) {
            lines.addAll(
                    renderer.render(iter.next()).stream()
                            .map(KotlinRenderingUtilities::kotlinIndent)
                            .collect(Collectors.toList()));
            if (iter.hasNext()) {
