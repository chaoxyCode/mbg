...
public class JavaBeansUtilTest {
    ...
    public void testGetValidPropertyName() {
        ...
        assertEquals(
                "i_PARAM_INT_1",
                getValidPropertyName("I_PARAM_INT_1")); // $NON-NLS-1$ //$NON-NLS-2$
        assertEquals("_fred", getValidPropertyName("_fred")); // $NON-NLS-1$ //$NON-NLS-2$
        assertEquals(
                "accountType", getValidPropertyName("AccountType")); // $NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public void testGetGetterMethodName() {
        assertEquals(
                ...
                getGetterMethodName(
                        "eMail",
                        FullyQualifiedJavaType.getStringInstance())); // $NON-NLS-1$ //$NON-NLS-2$
        assertEquals(
                "getFirstName",
                getGetterMethodName(
                        "firstName",
        ...
        assertEquals(
                ...
                getGetterMethodName(
                        ...
                        FullyQualifiedJavaType
                                .getBooleanPrimitiveInstance())); //$NON-NLS-1$ //$NON-NLS-2$
        assertEquals(
                "getI_PARAM_INT_1",
                getGetterMethodName(
                        "i_PARAM_INT_1",
    ...
    public void testGetSetterMethodName() {
        ...
        assertEquals(
                "setI_PARAM_INT_1",
                getSetterMethodName("i_PARAM_INT_1")); // $NON-NLS-1$ //$NON-NLS-2$
        assertEquals("set_fred", getSetterMethodName("_fred")); // $NON-NLS-1$ //$NON-NLS-2$
        assertEquals(
                "setAccountType", getSetterMethodName("AccountType")); // $NON-NLS-1$ //$NON-NLS-2$
    }
}
