...
public class FragmentGenerator {
        ...
        for (IntrospectedColumn column : introspectedTable.getPrimaryKeyColumns()) {
            ...
            builder.withParameter(
                    new Parameter(
                            ...
                            column.getJavaProperty() + "_")); // $NON-NLS-1$
            if (first) {
    ...
    public MethodParts getAnnotatedConstructorArgs() {
        ...
        builder.withImport(
                new FullyQualifiedJavaType("org.apache.ibatis.type.JdbcType")); // $NON-NLS-1$
        builder.withImport(
                new FullyQualifiedJavaType(
                        "org.apache.ibatis.annotations.ConstructorArgs")); //$NON-NLS-1$
        builder.withImport(
                new FullyQualifiedJavaType("org.apache.ibatis.annotations.Arg")); // $NON-NLS-1$
        ...
        Iterator<IntrospectedColumn> iterNonPk =
    ...
    private String getArgAnnotation(
    ...
    public MethodParts getAnnotatedResults() {
        ...
        builder.withAnnotation(
                "@Results(id=\"" + resultMapId + "\", value = {"); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    private String getResultAnnotation(
            Set<FullyQualifiedJavaType> imports,
            IntrospectedColumn introspectedColumn,
            boolean idColumn) {
        StringBuilder sb = new StringBuilder();
                ...
                .ifPresent(
                        introspectedColumn -> {
                            if (gk.isJdbcStandard()) {
                                builder.withImport(
                                        new FullyQualifiedJavaType(
                                                "org.apache.ibatis.annotations.Options")); //$NON-NLS-1$
                                sb.append(
                                        "@Options(useGeneratedKeys=true,keyProperty=\"record."); //$NON-NLS-1$
                                sb.append(introspectedColumn.getJavaProperty());
                                sb.append("\")"); // $NON-NLS-1$
                                builder.withAnnotation(sb.toString());
                            } else {
                                builder.withImport(
                                        new FullyQualifiedJavaType(
                                                "org.apache.ibatis.annotations.SelectKey")); //$NON-NLS-1$
                                FullyQualifiedJavaType fqjt =
    ...
    public List<String> getSetEqualLines(List<IntrospectedColumn> columnList, boolean terminate) {
        ...
        List<IntrospectedColumn> columns =
                ListUtilities.removeIdentityAndGeneratedAlwaysColumns(columnList);
    ...
    public List<String> getSetEqualLinesV2(
    ...
    public List<String> getSetEqualWhenPresentLines(
            List<IntrospectedColumn> columnList, boolean terminate) {
        List<String> lines = new ArrayList<>();
        List<IntrospectedColumn> columns =
                ListUtilities.removeIdentityAndGeneratedAlwaysColumns(columnList);
        Iterator<IntrospectedColumn> iter = columns.iterator();
    ...
    public List<String> getSetEqualWhenPresentLinesV2(
            List<IntrospectedColumn> columnList,
            String firstLinePrefix,
            String subsequentLinePrefix,
            boolean terminate) {
