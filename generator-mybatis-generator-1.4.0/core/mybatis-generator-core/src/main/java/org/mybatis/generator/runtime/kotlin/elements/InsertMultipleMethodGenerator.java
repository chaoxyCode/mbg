...
public class InsertMultipleMethodGenerator extends AbstractKotlinFunctionGenerator {
    ...
    public KotlinFunctionAndImports generateMethodAndImports() {
        ...
        // Kotlin type inference gets lost if we don't name the helper method something different
        // from the
        // regular mapper method
        String mapperMethod =
                Utils.generateMultipleRowInsertHelper(introspectedTable)
                        ? "insertMultipleHelper"
                        : "insertMultiple"; //$NON-NLS-1$ //$NON-NLS-2$

        KotlinFunctionAndImports functionAndImports =
                KotlinFunctionAndImports.withFunction(
                                KotlinFunction.newOneLineFunction(
                                                mapperName + ".insertMultiple") // $NON-NLS-1$
                                        .withArgument(
                                                KotlinArg.newArg("records") // $NON-NLS-1$
                                                        .withDataType(
    ...
    public boolean callPlugins(KotlinFunction kotlinFunction, KotlinFile kotlinFile) {
        return context.getPlugins()
                .clientInsertMultipleMethodGenerated(kotlinFunction, kotlinFile, introspectedTable);
    }
