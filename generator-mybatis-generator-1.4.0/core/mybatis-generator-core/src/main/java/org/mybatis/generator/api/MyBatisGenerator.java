...
/**
 * This class is the main interface to MyBatis generator. A typical execution of the tool involves
 * these steps:
 *
 * <ol>
 *   <li>Create a Configuration object. The Configuration can be the result of a parsing the XML
 *       configuration file, or it can be created solely in Java.
 *   <li>Create a MyBatisGenerator object
 *   <li>Call one of the generate() methods
 * </ol>
 *
 * @author Jeff Butler
...
public class MyBatisGenerator {
     ...
     * @param shellCallback an instance of a ShellCallback interface. You may specify <code>null
     *     </code> in which case the DefaultShellCallback will be used.
     * @param warnings Any warnings generated during execution will be added to this list. Warnings
     *     do not affect the running of the tool, but they may affect the results. A typical warning
     *     is an unsupported data type. In that case, the column will be ignored and generation will
     *     continue. You may specify <code>null</code> if you do not want warnings returned.
     ...
     * provided and the method can be canceled through the ProgressCallback interface. This version
     * of the method runs all configured contexts.
     *
     ...
     * @param callback an instance of the ProgressCallback interface, or <code>null</code> if you do
     *     not require progress information
     * @param contextIds a set of Strings containing context ids to run. Only the contexts with an
     *     id specified in this list will be run. If the list is null or empty, than all contexts
     *     are run.
     * @throws SQLException the SQL exception
     * @throws IOException Signals that an I/O exception has occurred.
     * @throws InterruptedException if the method is canceled through the ProgressCallback
     ...
     *     id specified in this list will be run. If the list is null or empty, than all contexts
     *     are run.
     * @param fullyQualifiedTableNames a set of table names to generate. The elements of the set
     *     must be Strings that exactly match what's specified in the configuration. For example, if
     ...
     *     If the Set is null or empty, then all tables in the configuration will be used for code
     *     generation.
     * @param writeFiles if true, then the generated files will be written to disk. If false, then
     *     the generator runs but nothing is written
     * @throws SQLException the SQL exception
     * @throws IOException Signals that an I/O exception has occurred.
     * @throws InterruptedException if the method is canceled through the ProgressCallback
     */
    ...
    private void writeGeneratedJavaFile(GeneratedJavaFile gjf, ProgressCallback callback)
        ...
        try {
            File directory =
                    shellCallback.getDirectory(gjf.getTargetProject(), gjf.getTargetPackage());
            targetFile = new File(directory, gjf.getFileName());
            if (targetFile.exists()) {
                if (shellCallback.isMergeSupported()) {
                    source =
                            shellCallback.mergeJavaFile(
                    ...
                    warnings.add(
                            getString(
                                    "Warning.11", //$NON-NLS-1$
                                    targetFile.getAbsolutePath()));
                } else {
                    source = gjf.getFormattedContent();
                    targetFile = getUniqueFileName(directory, gjf.getFileName());
                    warnings.add(
                            getString("Warning.2", targetFile.getAbsolutePath())); // $NON-NLS-1$
                }
            } else {
                source = gjf.getFormattedContent();
            }
    ...
    private void writeGeneratedKotlinFile(GeneratedKotlinFile gkf, ProgressCallback callback)
        ...
        try {
            File directory =
                    shellCallback.getDirectory(gkf.getTargetProject(), gkf.getTargetPackage());
    ...
    private void writeGeneratedXmlFile(GeneratedXmlFile gxf, ProgressCallback callback)
        ...
        try {
            File directory =
                    shellCallback.getDirectory(gxf.getTargetProject(), gxf.getTargetPackage());
    ...
    private File getUniqueFileName(File directory, String fileName) {
        ...
        if (answer == null) {
            throw new RuntimeException(
                    getString("RuntimeError.3", directory.getAbsolutePath())); // $NON-NLS-1$
        }
    ...
    /**
     * Returns the list of generated Kotlin files after a call to one of the generate methods. This
     * is useful if you prefer to process the generated files yourself and do not want the generator
